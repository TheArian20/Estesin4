document.addEventListener("DOMContentLoaded", async () => {
  ...
  function renderizar(fecha = "todas") {
    ...
    lista.innerHTML = visibles.map((sesion) => `<article class="agenda-card"><span>${textoDia(sesion.fecha)}</span><strong>${sesion.materia}</strong><small>${sesion.hora}${sesion.detalle ? ` · ${sesion.detalle}` : ""}</small></article>`).join("");
  }
