(function () {
  ...
  class Consulta {
    ...
    async filas(){const s=await db.collection(this.tabla).get();let f=s.docs.map(d=>({id:d.id,...normalizar(d.data())})).filter(x=>this.coincide(x));for(const[c,a]of[...this.ordenes].reverse())f.sort((x,y)=>x[c]===y[c]?0:x[c]==null?1:y[c]==null?-1:(x[c]>y[c]?1:-1)*(a?1:-1));if(this.maximo!=null)f=f.slice(0,this.maximo);return f}
    async ejecutar(){try{
  ...
  window.STESIN_SUPABASE=cliente; window.STESIN_DATOS=cliente;
